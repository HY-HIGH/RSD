...
def visualize_boxes_and_labels_on_image_array(
  ...
  count = 0#박스의 수 카운트 
  box_size = 0 #박스의 크기
  if class_name == 'person':#class_name 이 사람이면 
    for box, color in box_to_color_map.items():#박스의 수 만큼 실행된다.
      ymin, xmin, ymax, xmax = box
      xmid += (xmin+xmax)/2 #중심값을 더해준다. (여러개의 입력 받기 위함.)
      ymid += (ymin+ymax)/2
      ...
      #print(display_str)
      #print("box_size:%f"%(box_size))
      #print(class_name)
  if count != 0:   
         print("xmid: %f, ymid: %f," %(xmid/count,ymid/count))#박스의 개수로 나누어준다 들여쓰기 조심!
         print(display_str)
         print("box_size:%f"%(box_size/count))
